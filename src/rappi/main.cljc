(ns rappi.main
  "Kotodama WASM entrypoint for the Rappi clean-room actor (L4) — Clojure port.
  ...
  facts (`rappi.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"rappi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rappi-compat" :tier tier :entities entities} 200])
